...
def _deepcrop(img):
    w,h=img.size
    return img.crop((int(0.08*w),int(0.05*h),int(0.92*w),int(0.60*h)))  # deep-secondary band
...
print(f"device={device.type}  classes={classes}  mode=FINE-TUNE (layer4+fc)  "
      f"lr={args.lr}  epochs={args.epochs}  seed={args.seed}  INPUT_RES={RES}  CROP=deep-secondary[y5-60,x8-92]", flush=True)
