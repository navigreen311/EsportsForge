...
ap.add_argument("--resize", type=int, default=320, help="input square resolution")
ap.add_argument("--crop", type=str, default="0.08,0.05,0.92,0.60", help="x0,y0,x1,y1 fractions")
args = ap.parse_args()
RES = args.resize
CB=[float(x) for x in args.crop.split(",")]
def _deepcrop(img):
    w,h=img.size
    return img.crop((int(CB[0]*w),int(CB[1]*h),int(CB[2]*w),int(CB[3]*h)))

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
train_tf = transforms.Compose([transforms.Lambda(_deepcrop), transforms.Resize((RES,RES)), transforms.RandomHorizontalFlip(),
                               transforms.ColorJitter(0.2,0.2,0.2), transforms.ToTensor(),
                               transforms.Normalize(IMAGENET_MEAN, IMAGENET_STD)])
eval_tf = transforms.Compose([transforms.Lambda(_deepcrop), transforms.Resize((RES,RES)), transforms.ToTensor(),
                              transforms.Normalize(IMAGENET_MEAN, IMAGENET_STD)])
...
print(f"device={device.type}  classes={classes}  mode=FINE-TUNE (layer4+fc)  "
      f"lr={args.lr}  epochs={args.epochs}  seed={args.seed}  INPUT_RES={RES}  CROP={args.crop}", flush=True)
