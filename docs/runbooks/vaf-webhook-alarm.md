...
- **Deploy + drill** — require a live AWS environment (credentials, CloudWatch receiving the metric, the stack deployed). **Evidence-confirmed blocked here (Session #11):** no `aws` CLI, no boto3, `AWS_REGION`/`AWS_PROFILE` unset, no `~/.aws/`. Not available in local dev. The end-to-end **drill** (force >0.1% failure → alarm → page) must be exercised in that environment; it has not been run.
