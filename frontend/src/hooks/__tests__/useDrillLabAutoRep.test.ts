...
import { useDrillLabAutoRep } from '../useDrillLabAutoRep';
...
test('one FORMATION_LOCKED → exactly one rep (real shape)', () => {
  const onRep = jest.fn();
  renderHook(({ lastEvent }) => useDrillLabAutoRep({ lastEvent, onRep }), {
...
test('does not double-count a re-surfaced event_id (dedupe, load-bearing)', () => {
  ...
  const { rerender } = renderHook(({ lastEvent }) => useDrillLabAutoRep({ lastEvent, onRep }), {
...
test('flag-off / no events surfaced (null lastEvent) → no auto-reps', () => {
  const onRep = jest.fn();
  const { rerender } = renderHook(({ lastEvent }) => useDrillLabAutoRep({ lastEvent, onRep }), {
